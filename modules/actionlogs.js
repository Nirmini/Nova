...
async function sendActionLog(guild, eventType, description) {
    ...
    await webhook.send({
        username: "Nova",
        avatarURL: "https://nirmini.dev/imgs/Nova/v4/Nightsmith-V3Logo-Dev.png",
        embeds: [embed]
    });
}
...
client.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
    ...
    const authorTag = oldMessage.author?.tag || '[[undefined]]';
    const beforeContent = oldMessage.content || '[[undefined]]';
    const afterContent = newMessage.content || '[[undefined]]';

    await sendActionLog(oldMessage.guild, "messageEdit",
        `A message by **${authorTag}** was edited in <#${oldMessage.channel.id}>.\n**Before:** ${beforeContent}\n**After:** ${afterContent}`
    );
});
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
    ...
    if (error.length > 1024) {
        discorderror = error.substring(0, 1020) + "..."; // Add an extra character for safety.
    } else discorderror = error
    log.whook("[ACTIONLOG ERR]:" + discorderror)
