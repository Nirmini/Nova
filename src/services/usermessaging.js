...
router.post('/api/sendmsg', async (req, res) => {
    const { type, userId, guildId, channelId, messageType, content, embed, components, rawMessage, devUserId } = req.body;
    ...
    try {
        if (type === 'dm') {
            ...
            // Raw JSON mode (from JSON Import tab)
            if (messageType === 'raw' && rawMessage) {
                const sendOptions = {};
                if (rawMessage.content) sendOptions.content = rawMessage.content;
                if (rawMessage.embeds?.length) sendOptions.embeds = rawMessage.embeds;
                if (rawMessage.components?.length) sendOptions.components = rawMessage.components;
                if (rawMessage.flags !== undefined) sendOptions.flags = rawMessage.flags;
                await user.send(sendOptions);
                console.log("[UMS]: Sent raw DM");
            ...
            if (messageType === 'raw' && rawMessage) {
                ...
                if (rawMessage.content) sendOptions.content = rawMessage.content;
                if (rawMessage.embeds?.length) sendOptions.embeds = rawMessage.embeds;
                if (rawMessage.components?.length) {
                    sendOptions.flags = MessageFlags.IsComponentsV2;
                    sendOptions.components = rawMessage.components;
                }
                if (rawMessage.flags !== undefined) sendOptions.flags = rawMessage.flags;
