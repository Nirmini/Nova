// THIS ENTIRE APP IS GETTING REDONE EVENTUALLY. (IT SUCKS.)
#include <windows.h>
#include <iostream>
#include <string>
#include <dwmapi.h>
...
int main() {
    ...
    size_t p = exeDir.find_last_of("\\/");
    if (p != std::string::npos) parentDir = exeDir.substr(0, p);

    // Fallback absolute path if parent doesn't exist or fails
    const std::string fallbackDir = "C:\\Users\\Owner\\Documents\\MultiBot-Dev";

    BOOL setOk = FALSE;
    if (!parentDir.empty()) {
        setOk = SetCurrentDirectoryA(parentDir.c_str());
    }
    if (!setOk) {
        // try exeDir itself
        setOk = SetCurrentDirectoryA(exeDir.c_str());
    }
    if (!setOk) {
        // final fallback to user-specified working dir
        setOk = SetCurrentDirectoryA(fallbackDir.c_str());
    }
    if (!setOk) {
        // report but continue; CreateProcess will inherit whatever working dir remains
        DWORD err = GetLastError();
        std::cerr << "Warning: failed to set working directory (parent/exe/fallback). Error: " << err << std::endl;
    }
